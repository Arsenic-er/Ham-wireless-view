...
2026-07-31 的工作区新增私有 validation 平台天地图在线同源代理、右下动态公制比例尺和 MapLibre desired-state 重放。浏览器不接触 `tk` 或上游 URL；token 由项目运行目录中的 `0600` 私密文件管理。当前未配置 token，尚未请求真实瓦片或完成浏览器烟雾；该在线验证路径也不提供离线底图或正式地图导出，审图号、服务/离线/导出授权仍是公开发布门槛。
