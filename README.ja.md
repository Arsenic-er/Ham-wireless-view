...
- 現在の製品方針では、表示用ベースマップをオンライン専用としています。validation 版は同一オリジンのプロキシを介して Tianditu ベクターマップと EOxCloudless 衛星画像を利用でき、オンラインベースマップが利用できない場合は WGS84 座標グリッドへフォールバックします。実 DEM/WBM + ITM 計算、セッション内で最大 8 個のカバレッジレイヤー、動的縮尺、ブラウザー内の診断用 PNG/PDF エクスポートは引き続き利用できます。
- -140..-60 dBm、1 dB 刻みの全体表示しきい値スライダーを実装済みです。弱いピクセルを動的に非表示にしますが、伝搬計算の再実行や統計、本回のエクスポート内容の変更は行いません。現在、フロントエンド 145 件、Rust workspace 131 件のテスト（環境依存の 5 件は除外）と、実キャッシュを使う 3 組の HTTP スモークテストに合格しています。8 レイヤー時のサーバー CPU マイクロベンチマークは P95 5.982 ms です。Codex Windows ACL 障害のため管理ブラウザーでのドラッグ操作は未検証で、Windows WebView2 実機試験も未完了です。
- 旧 4 省 PMTiles ルートは製品対象から外れ、過去の技術証拠としてのみ残しています。EOxCloudless は validation 用のオンライン衛星レイヤーであり、公開 Windows リリース資産には含めません。

- 現在のソースには、1–200 km の経路を対象とする独立したオンライン 2 地点 TX/RX リンク解析モードを実装済みです。地図上で TX と RX を選ぶと、地形／フレネルプロファイルと計画用分類を表示します。管理下の validation プロセスは、このコードで再ビルド／再起動しておらず、公開サービスも展開していません。

### Windows Alpha

- Windows/Tauri はオンラインの Tianditu ベクターマップと衛星画像に対応しています。ユーザーが個人用 `tk` を設定し、Windows は現在のユーザーの DPAPI で暗号化して保存します。オンラインタイルは 2.5 GB キャッシュにも診断エクスポートにも入りません。Alpha 2 では明示的な接続テストを追加し、「設定を保存済み」と「オンライン地図へ接続可能」を区別しています。テスト時にタイルキャッシュは書き込みません。
...
- ライト／ダーク UI。
- Windows デスクトップではオンライン Tianditu ベクター／衛星ベースマップを使用。個人用 `tk` は DPAPI で暗号化し、タイルは永続化せずエクスポートにも含めません。
