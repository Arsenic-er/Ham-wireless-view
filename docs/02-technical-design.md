...
- 状态：应用骨架、空白合规占位地图、单点与 200 km 圆、参数预设、浅/深/系统主题、固定色标、Rust 输入归一化、真实缓存计算、区域下载、缓存管理、内部 PNG/PDF 诊断报告、Windows 交叉编译和离线 NSIS 产物已完成；合规有效区、结果栅格最终重投影、正式地图导出、签名和 Windows 10/11 完整实机验收尚未完成。
...
- 当前 MapLibre `image` 四角覆盖叠加的内部几何误差评估；若超过 1 km 输出容差，必须在 Rust 侧重投影到 Web Mercator 栅格后再进入发布版。
