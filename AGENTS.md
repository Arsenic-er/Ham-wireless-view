...
- The Windows online basemap uses Tauri's WebView2-compatible `http://*.localhost/{layer}/{z}/{x}/{y}` custom-protocol mapping; a user-owned Tianditu key is optional and must never be exposed to the WebView, URL, log, bootstrap payload, or Git.
